...
export default function TrainingCoursesApp() {
  ...
  async function handleCourseAction(course: CourseType) {
    if (course.isFree) {
      setStatus(`تم فتح الدورة المجانية: ${course.title}`)
      return
    }
    ...
    if (!piUser) {
      setStatus("سجل الدخول أولًا قبل الاشتراك")
      return
    }
    ...
    try {
      setBusy("pay")
      setStatus(`جاري بدء الدفع للدورة: ${course.title}`)
      ...
      await window.Pi.createPayment(
        ...
        },
        {
          ...
          onReadyForServerCompletion: async (paymentId: string, txid: string) => {
            ...
            setStatus(`تم الاشتراك بنجاح في: ${course.title}`)
          },
  ...
  const featuredCourses: CourseType[] = [
    {
      ...
      isNew: true,
      cta: "اشترك الآن",
      memo: "الاشتراك في دورة React Native",
    },
    {
      id: 2,
    ...
    },
    {
      ...
      isNew: false,
      cta: "اشترك الآن",
      memo: "الاشتراك في دورة التسويق الرقمي",
    },
  ]
        ...
        <CardContent className="p-0">
          ...
          <div className="space-y-4 p-5 text-right">
            ...
            <div className="flex items-end justify-between gap-4">
              ...
              <Button
                className="h-14 rounded-2xl px-6 text-xl font-bold"
                disabled={busy === "pay"}
                onClick={() => handleCourseAction(course)}
              >
                {busy === "pay" ? "جاري المعالجة..." : course.cta}
              </Button>
            </div>
          </div>
        </CardContent>
